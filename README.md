...
- `WA_COMMON_PROXY`，可选静态出站代理 URL；配置后长连接、联系人同步、头像下载、注册/检测和账号设置统一使用该代理，未配置时直连降级

协议 endpoint、WASafe 公钥、WA app 版本、User-Agent、ChatD host/TLS/timeout、请求 timeout、本地 profile 目录和设备指纹策略都内置在 wa-app 原生 engine 中，不再作为运行时环境变量暴露；代理只是可选增强，未配置时不阻塞服务。
